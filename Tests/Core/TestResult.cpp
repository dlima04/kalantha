/*
* Copyright (c) 2025 Diago Lima
* SPDX-License-Identifier: BSD-3-Clause
*/

#define KTA_ASSUME_TESTING_ENV_
#include <catch2/catch_test_macros.hpp>
#include <Kalantha/Core/Result.hpp>
#include <Kalantha/Core/Try.hpp>
#include <Kalantha/Meta/TypeTraits.hpp>
#include <string>

/*
* Unit tests here are minimal.
*/

using namespace kta;

struct CTORHelper {
  int x_{};
  int y_{};
  constexpr auto operator<=>(const CTORHelper&) const = default;
  constexpr CTORHelper() = default;
  constexpr CTORHelper(int x, int y) : x_(x), y_(y) {}
};

struct DTORHelper {
  int& ref_;
  DTORHelper(int& ref) : ref_(ref) {}
  ~DTORHelper() { ref_ += 1; }
};

TEST_CASE("Construct", "[Core.Result]") {
  Result<CTORHelper, int> obj1(300, 400);
  Result<CTORHelper, int> obj2 = 3;
  auto obj3 = Result<CTORHelper, int>::create(100, 200);

  REQUIRE(obj1.has_value());
  REQUIRE(obj3.has_value());
  REQUIRE(!obj2.has_value());

  REQUIRE(obj1->x_ == 300);
  REQUIRE(obj1->y_ == 400);
  REQUIRE(obj3->x_ == 100);
  REQUIRE(obj3->y_ == 200);

  auto val = obj3.value_or(CTORHelper(420, 69));
  REQUIRE(val == obj3.value());
}

TEST_CASE("Destroy", "[Core.Result]") {
  int num = 20;
  {
    Result<DTORHelper, const char*> obj1(static_cast<int&>(num));
    REQUIRE(obj1.has_value());
  }

  REQUIRE(num > 20);
  Result<int, const char*> obj2 = 33;
  auto err = obj2.error_or("blabla");
  REQUIRE(std::string(err) == "blabla");
}

TEST_CASE("Try", "[Core.Result]") {
  int num = 20;
  auto func1 = []() -> Result<int, const char*> {
    return "failure.";
  };

  auto func2 = [&num, &func1]() -> Result<int, const char*> {
    auto val = TRY(func1());
    num += 2;
    return val;
  };

  auto res = func2();
  REQUIRE(num == 20); /// Must not have been altered.
  REQUIRE(!res.has_value());
}
